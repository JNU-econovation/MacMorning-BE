...
class ChromaRepository(BaseVectorRepository):
    ...
    def add_single_document(self, document: Document, book_id: str) -> None:
        if not document or not document.page_content:
            print("문서가 비어있어서 저장하지 않습니다.")
            return
        
        try:
            print(f"ChromaDB에 문서 추가 시작 - book_id: {book_id}")
            collection = self._get_or_create_collection(book_id)
            print(f"컬렉션 준비 완료: {collection.name}")
            ...
            print(f"임베딩 생성 중... (텍스트 길이: {len(text)})")
            # 임베딩 생성
            ...
            print(f"ChromaDB 저장 완료 - doc_id: {doc_id}")
            
        except Exception as e:
            print(f"단일 문서 추가 중 오류: {str(e)}")
            import traceback
            traceback.print_exc()
            raise
